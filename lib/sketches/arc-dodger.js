...
export default function arcDodger({ p5, width, height, scoreBoard }) {
  let target = null;
  let pies = null;
  let center = null;
  let gapAngle = 0;
  ...
  p5.draw = () => {
    ...
    if (justDied) {
      play = false;
      scoreBoard.finishWithScore(Math.round(time * 100) / 100);
    }
  };
